...
- 背景使用 React Bits Galaxy，跟随深浅色；减少动态效果时显示静态星空，后台标签页暂停渲染，WebGL 不可用时回退到纯色背景。

- 输入时按名称和域名匹配书签，最多显示 5 条。点击结果或方向键选择后回车打开书签；直接回车仍联网搜索或前往网址，Esc 收起结果。

- 搜索框上方显示设备本地日期、星期和时间，每秒更新；时钟使用改编的 React Bits Split Flap Text，仅变化的数字翻牌，减少动态效果偏好下即时更新。
