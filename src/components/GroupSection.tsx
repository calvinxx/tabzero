...
export default function GroupSection({ group, index }: { group: BookmarkGroup; index: number }) {
  return <section className="bookmark-group" style={{ '--group-offset': `${index * 80}ms` } as CSSProperties} aria-labelledby={`group-${index}`}>
    <div className="group-heading"><h2 id={`group-${index}`}><BlurText text={group.name} /></h2><span className="group-count">{group.links.length.toString().padStart(2, '0')}</span></div>
